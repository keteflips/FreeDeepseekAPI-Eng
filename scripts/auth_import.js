...
Options:
  --input, -i   Source JSON: existing deepseek-auth.json or browser cookie export
  --output, -o  Target auth path (default: ${DEFAULT_OUT})

Security:
  For cookies.json pass the token via DEEPSEEK_TOKEN, not as a CLI argument,
  so it does not leak into shell history/process list.

VPS flow:
  1) On the local PC: npm run auth
  2) Copy deepseek-auth.json to the VPS
  3) On the VPS: npm run auth:import -- --input ./deepseek-auth.json
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (errors.length) {
    ...
    console.error('[auth:import] If you are importing browser cookies, pass the token via DEEPSEEK_TOKEN=...');
    return 2;
  }
